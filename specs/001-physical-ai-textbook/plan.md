...
*   **Syntax Highlighting Theme**: `prism-dark` (or a similar dark theme for code blocks).
...
*   **Sidebar Generation Strategy**: Primarily `autogenerated` using `@docusaurus/plugin-content-docs` from the file system structure within the `docs/` folder, respecting `sidebar_position` and `slug` frontmatter in MDX files. For advanced custom ordering or grouping not achievable through file system and frontmatter, a `sidebars.js` file *could* be introduced, but for this project, the `autogenerated` approach is preferred for simplicity and maintainability.
...
The content generation will strictly follow the Spec-Kit Plus → Claude Code workflow:
